...
class ManageAction extends Action {
    ...
    private function add(){
        if(isset($_POST['send'])){
            ...
            if(Validate::checkEquals($_POST['admin_pass'],$_POST['admin_notpass'])) Tool::alertBack('警告，密码必须与密码确认一致！');
            $this->_model->_admin_user = $_POST['admin_user'];
            if($this->_model->getOneManage()) Tool::alertBack('警告，此用户被占用！');
            $this->_model->_admin_pass = sha1($_POST['admin_pass']);
            $this->_model->_level = $_POST['level'];
            $this->_model->addManage() ? Tool::alertLocation('恭喜你，新增数据成功！','manage.php?action=show') : Tool::alertBack('新增数据失败！');
        }
    ...
    private function update(){
        if(isset($_POST['send'])){
            ...
            $this->_model->_level=$_POST['level'];
            $this->_model->updateManage() ? Tool::alertLocation('恭喜你修改管理员成功！',@$_POST['prev_url']) : Tool::alertBack('很遗憾，修改管理员失败！');
        }
        if(isset($_GET['id'])){
    ...
    private function delete(){
        if(isset($_GET['id'])){
            $this->_model->_id=$_GET['id'];
            $this->_model->deleteManage() ? Tool::alertLocation('恭喜你，删除管理员成功！',PREV_URL) : Tool::alertBack('很遗憾删除管理员失败！');
        }else{
            Tool::alertBack('非法操作！');
        }
    }
